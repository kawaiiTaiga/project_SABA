...
inline String topicClaimDev()    { return "mcp/dev/" + device_id + "/claim"; }
...
void startRuntime() {
  ...
  mqtt.setCallback([](char* topic, byte* payload, unsigned length){
    ...
    // 0) Claim 처리 (토큰 수신)
    if (t == topicClaimDev()) {
      if (CFG.secret_token.length() > 0) {
        Serial.println("[CLAIM] Already claimed, ignoring new token.");
        return;
      }
      StaticJsonDocument<256> doc;
      ...
      if (token && strlen(token) > 0) {
        CFG.secret_token = String(token);
        prov->save(CFG);
        Serial.printf("[CLAIM] Device claimed! Token saved (len=%u)\n", CFG.secret_token.length());
      }
      return;
    }
    ...
    if (t == topicPortsSetDev()) {
      ...
      String parsed;
      serializeJson(doc, parsed);

      const char* portName = doc["port"].as<const char*>();
    ...
    if (t == topicCmdDev()) {
      ...
      // HMAC 검증
      ToolJob job;
      // Payload 처리 (Wrapped or Raw)
      StaticJsonDocument<1024> doc;
      DeserializationError err = deserializeJson(doc, payload, length);
      
      const char* dataStr = doc["data"];
      const char* sig     = doc["signature"];
      bool isWrapped = (err == DeserializationError::Ok && dataStr != nullptr && sig != nullptr);

      if (isWrapped) {
          // 1. Wrapped Payload (서명된 메시지)
          if (CFG.secret_token.length() > 0) {
              // [SEC] 검증 모드
              String calcSig = hmacSha256(CFG.secret_token, String(dataStr));
              if (calcSig != String(sig)) {
                  Serial.printf("[SEC] Signature mismatch! Exp=%s, Got=%s\n", calcSig.c_str(), sig);
                  
                  // [SEC] Error Response
                  StaticJsonDocument<512> inner;
                  if (deserializeJson(inner, dataStr) == DeserializationError::Ok) {
                      const char* rid = inner["request_id"];
                      if (rid) {
                          StaticJsonDocument<256> errDoc;
                          errDoc["request_id"] = rid;
                          errDoc["error"] = "wrong_token";
                          String errPayload;
                          serializeJson(errDoc, errPayload);
                          mqttPublishSafe(topicEvt(), errPayload, false);
                          Serial.printf("[SEC] Sent wrong_token error for rid=%s\n", rid);
                      }
                  }
                  return;
              }
              Serial.println("[SEC] Signature verified OK");
              
              // Timestamp 검증 (Inner JSON 파싱 필요)
              StaticJsonDocument<768> innerDoc;
              if (deserializeJson(innerDoc, dataStr) == DeserializationError::Ok) {
                  long ts = innerDoc["timestamp"] | 0;
                  time_t now = time(nullptr);
                  if (abs((long)now - ts) > 10) { // 10초 허용
                      Serial.printf("[SEC] Timestamp expired (dev=%ld, cmd=%ld), dropped\n", (long)now, ts);
                      return;
                  }
              }
          } else {
              // [SEC] Unclaimed 상태인데 서명된 메시지가 옴 -> 허용 (Unwrap)
              Serial.println("[SEC] Warning: Received signed msg but device not claimed. Unwrapping...");
          }

          // Unwrap: 내부 dataStr을 Job Payload로 사용
          size_t dataLen = strlen(dataStr);
          if (dataLen >= sizeof(ToolJob::payload)) {
              Serial.println("[MQTT] Inner payload too large, dropped");
              return;
          }
          memcpy(job.payload, dataStr, dataLen);
          job.payload[dataLen] = 0; // Null terminate
          job.len = dataLen;

      } else {
          // 2. Raw Payload (평문)
          if (CFG.secret_token.length() > 0) {
              // [SEC] Claimed 상태인데 평문이 옴 -> 거부 (보안 정책)
              Serial.println("[SEC] Error: Device is claimed but received unsigned payload. Dropped.");
              return;
          } else {
              // [SEC] Unclaimed 상태 -> 평문 허용
              Serial.println("[SEC] Accepting raw payload (unclaimed)");
              if (length >= sizeof(ToolJob::payload)) {
...
void ToolWorkerTask(void* pv) {
  ...
  for (;;) {
    ...
    if (rid) {
        bool isDup = false;
        for (int i=0; i<20; i++) {
            if (lastRids[i].equals(rid)) {
                isDup = true;
                break;
            }
        }
        if (isDup) {
            Serial.printf("[TOOL] Duplicate command ignored (rid=%s)\n", rid);
            continue;
        }
        // Cache new ID
