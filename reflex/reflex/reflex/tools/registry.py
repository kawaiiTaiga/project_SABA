...
class VirtualToolRegistry:
    """
    가상 툴 레지스트리
    
    여러 툴을 하나로 묶어 실행하는 '가상 툴'을 관리
    """
    def __init__(self, tool_manager):
        self.tool_manager = tool_manager  # 다른 툴을 호출하기 위해 필요
        self.tools: Dict[str, Callable] = {}
        self.tool_schemas: Dict[str, Dict] = {}
        self.virtual_tools_config: List[Dict] = []
        
    def load_virtual_tools(self, configs: List[Dict]):
        """가상 툴 설정 로드 및 생성"""
        self.virtual_tools_config = configs
        self.tools.clear()
        self.tool_schemas.clear()
        
        print(f"📦 Loading virtual tools...")
        
        for config in configs:
            name = config.get('name')
            description = config.get('description', 'Virtual Tool')
            execution_mode = config.get('mode', 'sequential') # sequential or parallel
            sub_tools = config.get('tools', [])
            
            if not name or not sub_tools:
                print(f"   ⚠️ Invalid virtual tool config: {name}")
                continue
                
            # 가상 툴 함수 생성
            tool_func = self._create_virtual_tool(name, sub_tools, execution_mode)
            ...
            self.tool_schemas[name] = {
                ...
                'parameters': {
                    'type': 'object',
                    'properties': {
                        # 가상 툴은 기본적으로 인자를 받아 하위 툴에 전달
                        # (구현 단순화를 위해 모든 인자를 하위 툴로 pass-through)
                        'arguments': {
                            'type': 'object',
                            'description': 'Arguments to pass to sub-tools'
                        }
                    }
                }
            }
            print(f"   ✓ [Virtual] {name} ({execution_mode}, {len(sub_tools)} tools)")
            
    def _create_virtual_tool(self, name: str, sub_tools: List[str], mode: str) -> Callable:
        """가상 툴 실행 함수 생성"""
        
        async def virtual_tool_func(**kwargs):
            print(f"\n🚀 Executing Virtual Tool: {name} ({mode})")
            
            # 실제 실행 시점에 툴 가져오기 (동적 로딩 지원)
            # sub_tools는 ['tool_a', 'registry.tool_b'] 형태
            
            # ToolManager를 통해 툴 함수들 확보
            # 주의: ToolManager.get_tools_for_reflex는 {fullname: func}를 반환
            # 여기서는 순서가 중요하므로 sub_tools 순서대로 매핑 필요
            
            available_tools_map = self.tool_manager.get_tools_for_reflex(sub_tools)
            
            # 실행할 툴 리스트 순서대로 정렬
            target_funcs = []
            for tool_name in sub_tools:
                # full name 찾기
                found = False
                for full_name, func in available_tools_map.items():
                    # registry.tool_name OR tool_name
                    if full_name == tool_name or full_name.endswith(f".{tool_name}"):
                        target_funcs.append((tool_name, func))
                        found = True
                        break
                
                if not found:
                    print(f"   ❌ Sub-tool not found: {tool_name}")
                    return {
                        'success': False,
                        'error': f"Sub-tool not found: {tool_name}"
                    }
            ...
            try:
                if mode == 'sequential':
                    for t_name, t_func in target_funcs:
                        print(f"   👉 [Seq] Executing {t_name}...")
                        res = await t_func(**kwargs)
                        results.append({
                            'tool': t_name,
                            'result': res
                        })
                        
                        # 실패 시 중단 여부? 일단 계속 진행한다고 가정하거나, 옵션으로?
                        # 여기서는 하나라도 실패하면 중단하지 않고 모두 실행 시도
                        
                elif mode == 'parallel':
                    print(f"   👉 [Par] Executing {len(target_funcs)} tools simultaneously...")
                    
                    # 병렬 실행 태스크 생성
                    tasks = [t_func(**kwargs) for _, t_func in target_funcs]
                    
                    # gather로 실행
                    parallel_results = await asyncio.gather(*tasks, return_exceptions=True)
                    
                    for i, res in enumerate(parallel_results):
                        t_name = target_funcs[i][0]
                        results.append({
                            'tool': t_name,
                ...
                # 결과 조합
                final_text = "\n".join([f"[{r['tool']}] {r.get('result', r.get('error'))}" for r in results])
                
                return {
                    'success': True,
                    'result': results, # 상세 결과
                    'text': final_text # 텍스트 요약
                }
                
            except Exception as e:
                return {
                    'success': False,
                    'error': f"Virtual tool execution failed: {e}"
                }
                
        # Docstring 등 메타데이터
        virtual_tool_func.__doc__ = f"Virtual Tool {name} ({mode})"
        virtual_tool_func._virtual = True
    ...
    async def load_tools_from_mcp(self):
        """
        ReflexEngine 호환성: 가상 툴은 이미 load_virtual_tools로 로드됨.
        하지만 인터페이스 통일을 위해 이 메서드가 호출될 수 있음.
        """
        # 이미 로드되어 있으므로 아무것도 하지 않거나 로그 출력
        # print("   (Virtual tools are managed via config, skipping MCP load)")
        pass
...
class ToolManager:
    ...
    def __init__(self):
        ...
        self.virtual_registry = VirtualToolRegistry(self)
        # virtual registry도 registries에 추가 (조회 등 인터페이스 통일)
        self.registries['virtual'] = self.virtual_registry
