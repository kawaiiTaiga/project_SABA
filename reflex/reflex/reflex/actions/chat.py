...
@ActionBase.register('chat')
class ChatAction(ActionBase):
    """
    Interactive Chat Action with LLM and Tool support.
    Loops until exit keyword is entered.
    """
    
    description = "Interactive CLI Chat with LLM and Tools"
    schema = {
        "system_prompt": {
            "type": "text",
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.model = config.get('model', 'claude-3-5-sonnet-20241022') # Default to sonnet kwaiiTaiga/project_SABA uses
    ...
    async def execute(
        ...
        async def _input(prompt: str) -> str:
            ...
            if ipc and ipc.clients:
                ...
                # Wait for input from IPC
                # We need a way to know WHICH reflex is asking.
                # ideally Dashboard sends {reflex_id: ..., content: ...}
                # For MVP, assuming single interactive session or simple queue
                return await ipc.get_input()
            else:
                # Local Fallback
                return await asyncio.to_thread(input, prompt)

        await _print(f"\nStarting Chat Session (Type '{self.exit_keyword}' to end)...", "bold green")
        await _print(f"   Model: {self.model}", "cyan")
            ...
            while True:
                ...
                for block in response.content:
                    if block.type == 'text':
                        ...
                        await _print(block.text, "blue") # Simple colored text for MVP
        ...
        return {
            ...
            "text": transcript_text  # Engine records this as 'Output'
        }
